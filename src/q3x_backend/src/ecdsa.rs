...
/// use q3x_backend::ecdsa::get_ecdsa_key_id_from_env;
///
...
pub async fn get_public_key(wallet_id: String, key_id: EcdsaKeyId) -> Result<[u8; 65], String> {
    ...
    let uncompressed_pub_key =
        match PublicKey::parse_slice(&res.public_key, Some(PublicKeyFormat::Compressed)) {
            Ok(key) => key.serialize(),
            Err(_) => {
                return Err("decompression public key failed: ".to_string());
            }
        };
...
pub async fn sign_message(
    ...
    let (res,): (SignWithEcdsaResponse,) = ic_cdk::api::call::call_with_payment(
        ...
        "sign_with_ecdsa",
        (request,),
        DEFAULT_ECDSA_SIGN_CYCLES,
    )
    .await
    .map_err(|e| format!("Failed to call sign_with_ecdsa {}", e.1))?;

    let mut signature = res.signature;
    let pub_key = get_public_key(wallet_id, key_id).await?;
...
pub async fn is_signature_valid(
    ...
    let signature_obj = Signature::parse_overflowing_slice(signature_without_rec_id.as_slice())
